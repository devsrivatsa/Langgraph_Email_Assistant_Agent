...
from ea.main.models import Email
            ...
            try:
                if not os.path.exists(_SECRETS_PATH):
                    raise FileNotFoundError(
                        "credentials.json missing! Download it from Google Cloud Console"
                    )
                flow = InstalledAppFlow.from_client_secrets_file(_SECRETS_PATH, _SCOPES)
                creds = flow.run_local_server(port=_PORT)
...
def fetch_group_emails(
    ...
    Args:
        email_address: The email address to fetch from
    ...
    try:
        ...
        # Create query for recent emails
        query = f"newer_than:{minutes_since}m"
        logger.info(f"Using query: {query}")
        
        # Request messages from Gmail API
